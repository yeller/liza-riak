...
(deftest put-with-merge-test
  (testing "put with merge before a value is there uses the default"
    (let [b (riak-store/connect-pb-test-bucket {:bucket-name "test-bucket3"
                                                :client client
                                                :merge-fn +})]
      (store/wipe b)
  ...
  (testing "put with merge merges a new value onto the previous one"
    (let [b (riak-store/connect-pb-test-bucket {:bucket-name "test-bucket2"
                                                :client client
                                                :merge-fn set/union
...
(deftest counter-test
  ...
  (testing "incrementing counters gets the incremented value"
    (let [b (riak-store/connect-pb-test-bucket {:bucket-name "test-bucket4"
                                                :client client
                                                :merge-fn set/union})]
      (store/wipe b)
